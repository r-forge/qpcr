...
\begin{description}
    \item[Well] Optional, this represents the position of the detector on a given plate. This information, if given, will be used to check the plates are of the same size and in order to plot a representation of the card to look for spatial effects and other potential problems.
...
If \code{Well} and \code{Plate ID} information are given, then these are used to populate the \code{exprs.well.order}, a new \code{assayDat
a} slot introduced in the \code{qPCRBatch} object, as detailed below in section \code{qPCRBatch}.
...
\code{qPCRBatch.qPCR} will be a \code{qPCRBatch} object with an exprs and exprs.well.order, as well as a phenoData slot which gets automatically populated in the same was as when using \code{read.affybatch}. More detail is given in the \code{qPCRBatch} section below.
...
It also allows the decisions on how to deal with the analysis and combination of technical replicates to be controlled by the user, either using the \pkg{NormqPCR} package, or otherwise.
